...
$baseFolder = 	'files/uploaded';
$title = 'File Manager : '.ucfirst($jenis);

if($jenis !== 'root'){	
	if(!is_dir('../'.$baseFolder.'/'.$idPengguna)){
		mkdir('../'.$baseFolder.'/'.$idPengguna, 0777);
	}
	
	$baseFolder .= '/'.$idPengguna;
}

$baseFolder .= '/';

//folder only for conflict offline and online
//contoh -> /namafolder
//$folder = '/';
...
$config = array(
	...
	|
	*/
	'base_url' => ((isset($_SERVER['HTTPS']) && $_SERVER['HTTPS'] == "on") ? "https" : "http"). "://". @$_SERVER['HTTP_HOST'].''.$folder,
	/*
	|--------------------------------------------------------------------------
